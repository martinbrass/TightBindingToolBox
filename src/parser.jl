module Parser
    ...
    function Wannier90_import_TB(file_name)
        ...
        H = TB_Hamiltonian{ComplexF64,Int}(local_dim,lattice_dim)
        ...
        for idxR in 1:n_hoppings
            ...
            add_hoppings!(H,R,h/rweights[idxR])
        end
        return H
    end
    ...
    function FPLO_import_TB(filename)
        ...
        H = TB_Hamiltonian{ComplexF64,Int}(n_WFs,3)
        ...
        while !eof(file)
            ...
            if occursin("Tij, Hij",line) 
                ...
                while !occursin("end Tij, Hij",line) 
                    ...
                    add_hoppings!(H,R_lattice,h0)
                    H.hoppings[R_lattice][i,j] = parse(Float64,s[4]) + im*parse(Float64,s[5])
                    line = readline(file)
                end
            end
        end
